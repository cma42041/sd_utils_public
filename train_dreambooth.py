            ...
            if args.external_captions:
              ...
              if os.path.exists(cptpth):
                ...
                if args.Style:
                  ...
                  instance_prompt = pt
            # sys.stdout.write(instance_prompt[:45])
            sys.stdout.flush()
...
def main():
        ...
        if cur_class_images < args.num_class_images:
            ...
            for example in tqdm(
                ...
                for ii, image in enumerate(images):
                    image.save(class_images_dir / f"{example['index'][ii] + cur_class_images}.jpg")
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if ((global_step % 25) == 0):
                print(f'Step: {global_step}, Loss: {loss_avg.avg.item():.4f}, lr: {lr_scheduler.get_last_lr()[0]:.3e}')
            ...
            if args.save_n_steps >= 1:
               if global_step < args.max_train_steps and global_step+1==i:
                  ...
                  if not os.path.exists(save_dir):
                     os.mkdir(save_dir)
                  inst = os.path.basename(save_dir)
                  inst=inst.replace(" ", "_")
                  ...
                  if accelerator.is_main_process:
                     ...
                     if not args.dump_only_text_encoder:
                     ...
                     if args.save_sample_prompt is not None:
                        scheduler = EulerAncestralDiscreteScheduler(beta_end=0.012,beta_start=0.00085, prediction_type=noise_scheduler.config.prediction_type)
                        ...
                        for img_idx_ii in range(args.n_save_sample):
                            ...
                            ims.append(image)
                        topim = np.hstack((ims[0],ims[1]))
                        botim = np.hstack((ims[2],ims[3]))
                        bothim = np.vstack((topim,botim))
                        bothim = Image.fromarray(bothim)
                        bbase = os.path.basename(os.path.dirname(sample_dir))
                        imname=f'{bbase}_grid.png'
                        bothim.save(os.path.join(sample_dir,imname))
                        all_im_dir = os.path.join(args.output_dir,'image_samples')
                        os.makedirs(all_im_dir,exist_ok=True)
                        bothim.save(os.path.join(all_im_dir,f'img{global_im_ct:04d}_{imname}'))
                        global_im_ct += 1
                        print(f'4 Image grid saved to: {imname}')
                        del pipe
                        if torch.cuda.is_available():
                            torch.cuda.empty_cache() 
    ...
    if accelerator.is_main_process:
      ...
      if args.save_sample_prompt is not None:
        ...
        for i in range(args.n_save_sample):
            image = pipe(args.save_sample_prompt, num_inference_steps=33, guidance_scale=7.5).images[0]
    ...
    save_dir = args.output_dir
    chkpth = os.path.join(args.output_dir,os.path.basename(args.output_dir)+'.ckpt')
    if not args.dump_only_text_encoder:
        subprocess.call('python /workspace/diffusers/scripts/convert_diffusers_to_original_stable_diffusion.py --model_path ' + save_dir + ' --checkpoint_path ' + chkpth + ' --half',shell=True)
